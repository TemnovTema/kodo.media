...
import { CommunityCarouselArticle } from "@/components/community-carousel-article";

type CommunityArticleShelfProps = {
  articles: Article[];
  rubrics: Rubric[];
};

export function CommunityArticleShelf({
  articles,
  rubrics,
}: CommunityArticleShelfProps) {
  ...
  useEffect(() => {
    ...
    const applyLayout = () => {
      ...
      track.style.setProperty("--community-card-width", `${Math.max(cardWidth, 0)}px`);
    };
  ...
  const scrollByPage = (direction: -1 | 1) => {
    ...
    const styles = window.getComputedStyle(track);
    const gap = Number.parseFloat(styles.gap || styles.columnGap || "0");
    const card = track.querySelector<HTMLElement>("[data-community-card]");
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          aria-label="Прокрутить статьи назад"
        ...
        <button
          ...
          aria-label="Прокрутить статьи вперёд"
          className="inline-flex h-10 w-10 items-center justify-center border border-[var(--color-border)] font-mono text-[0.82rem] text-[var(--color-text)] transition-colors hover:border-[var(--color-border-strong)] hover:bg-[rgba(255,255,255,0.03)] disabled:cursor-default disabled:opacity-35 disabled:hover:border-[var(--color-border)] disabled:hover:bg-transparent"
        >
          →
        </button>
      ...
      <div ref={trackRef} className="community-carousel" aria-label="Лента статей">
        {articles.map((article) => (
          <CommunityCarouselArticle
            key={article.slug}
            article={article}
