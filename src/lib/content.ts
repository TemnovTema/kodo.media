    ...
    sections: [
      {
        ...
        paragraphs: [
          ...
          "На раннем этапе это легко принять за свободу. Но пользователю не нужна коллекция удачных экранов: ему нужен продукт, который не заставляет заново учиться на каждой странице. Последовательность появляется только тогда, когда команда умеет сравнить новую версию с предыдущим решением и назвать, что именно стало лучше.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "Такой слой полезен и в небольшой команде. Его можно распределить между человеком, коротким списком решений и регулярным review-проходом. Важно, чтобы кто-то или что-то имело право сказать: решение эффектное, но оно не продолжает уже выбранный язык продукта.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "Фиксация не должна превращаться в том документации. Достаточно короткого файла, который отвечает на практические вопросы: что нельзя менять без обсуждения, где брать компоненты, как выглядит хороший результат и что проверить перед коммитом. Такой документ сокращает не творчество, а число случайных развилок.",
        ],
      },
      {
        title: "Редактирование — это цикл",
        paragraphs: [
          "После каждой заметной итерации полезно остановиться и проверить не только техническую корректность. Сохранились ли иерархия, плотность, тон заголовков, сценарий действия и логика переходов? Если ответ неочевиден, работа еще не закончена, даже когда страница уже выглядит готовой.",
          "Агент особенно хорошо работает внутри такого цикла: он быстро производит варианты, а редакторский слой удерживает критерии. Тогда скорость перестает быть источником визуального шума и становится способом быстрее проверить сильную гипотезу.",
          "В итоге редактор нужен не после генерации, а до нее, во время нее и при приемке. Это слой, который превращает отдельные промты в развивающийся продукт, а не в архив случайных удач.",
        ],
      },
    ],
  },
